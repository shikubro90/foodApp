    ...
    <section className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full" id="home">
      ...
      <div className="py-2 flex-1 flex items-center relative">
        <img
          ...
          alt="heroBg"
        />
        <div className="w-full h-full absolute top-0 left-0 px-32 py-4 flex flex-wrap items-center justify-center gap-5">
          {heropData &&
            heropData.map(({id,name,decp,imageSrc,price}) => {
              return (
                <div key={id} className="w-90 lg:w-192 min-w-[190px] p-2 bg-red-500 bg-cardOverlay backdrop-blur-md flex flex-col items-center justify-center rounded-md">
                  <img className="w-20 lg:w-40 -mt-10 lg:-mt-20" src={imageSrc} alt="I1" />
                  <p className="text-base lg:text-base font-semibold text-textColor mt-2 lg:mt-4">
                    {name}
                  </p>
                  <p className="text-[10] lg:text-sm text-lightTextGray font-semibold my-1 lg:my-3">
                    {decp}
                  </p>
                  <p className="text-sm font-semibold text-headingColor">
                    <span className="text-xs text-red-500">$</span> {price}
                  </p>
                </div>
              )
            })}
